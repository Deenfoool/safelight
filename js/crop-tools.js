(function(){
  ...
  function panelMarkup(){return `<div class="panel-card sl-crop-panel">
    ...
    <div class="sl-crop-section">
      ...
      <div class="sl-crop-ratios"><button type="button" class="btn ghost active" data-crop-ratio="free">Свободно</button><button type="button" class="btn ghost" data-crop-ratio="1:1">1:1</button><button type="button" class="btn ghost" data-crop-ratio="4:3">4:3</button><button type="button" class="btn ghost" data-crop-ratio="3:2">3:2</button><button type="button" class="btn ghost" data-crop-ratio="16:9">16:9</button><button type="button" class="btn ghost" data-crop-ratio="9:16">9:16</button></div>
  ...
  function installPanel(){
    ...
    if(legacy){legacy.insertAdjacentElement('afterend',panel);legacy.remove()}else $('sl-inspector-panels')?.appendChild(panel);bindPanel();return panel
  }
  ...
  function resizeFree(handle,start,dx,dy,min){
    let x=start.x,y=start.y,w=start.w,h=start.h;const isLeft=handle==='left'||handle==='tl'||handle==='bl',isRight=handle==='right'||handle==='tr'||handle==='br',isTop=handle==='top'||handle==='tl'||handle==='tr',isBottom=handle==='bottom'||handle==='bl'||handle==='br';
    if(isLeft){const nx=clamp(start.x+dx,0,start.x+start.w-min.w);w=start.w+(start.x-nx);x=nx}if(isRight)w=clamp(start.w+dx,min.w,1-start.x);if(isTop){const ny=clamp(start.y+dy,0,start.y+start.h-min.h);h=start.h+(start.y-ny);y=ny}if(isBottom)h=clamp(start.h+dy,min.h,1-start.y);return clampFree({x,y,w,h},min)
  }
  ...
  function resizeFixed(handle,start,dx,dy,min){
    const nr=normalizedRatio();if(!nr)return resizeFree(handle,start,dx,dy,min);const left=start.x,right=start.x+start.w,top=start.y,bottom=start.y+start.h,cx=start.x+start.w/2,cy=start.y+start.h/2;let next={...start};
    if(['tl','tr','bl','br'].includes(handle)){const anchorX=handle.includes('l')?right:left,anchorY=handle.includes('t')?bottom:top,pointerX=clamp((handle.includes('l')?left:right)+dx,0,1),pointerY=clamp((handle.includes('t')?top:bottom)+dy,0,1);let w=Math.abs(anchorX-pointerX),h=Math.abs(anchorY-pointerY);if(w/Math.max(h,.0001)>nr)w=h*nr;else h=w/nr;w=Math.max(w,min.w);h=Math.max(h,min.h);if(w/h>nr)w=h*nr;else h=w/nr;next.w=w;next.h=h;next.x=handle.includes('l')?anchorX-w:anchorX;next.y=handle.includes('t')?anchorY-h:anchorY}
    else if(handle==='left'||handle==='right'){let w=handle==='left'?right-clamp(left+dx,0,right-min.w):clamp(start.w+dx,min.w,1-left),h=w/nr;next.w=w;next.h=h;next.x=handle==='left'?right-w:left;next.y=cy-h/2}
    else{let h=handle==='top'?bottom-clamp(top+dy,0,bottom-min.h):clamp(start.h+dy,min.h,1-top),w=h*nr;next.h=h;next.w=w;next.y=handle==='top'?bottom-h:top;next.x=cx-w/2}return fitFixedToBounds(next,nr,min)
  }
