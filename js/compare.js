...
style.textContent=`
.compare-preview{position:absolute;inset:0;display:none;overflow:hidden;border-radius:2px;z-index:10;isolation:isolate}
.compare-preview.show{display:block}
.compare-preview .compare-after{position:absolute;inset:0;width:100%;height:100%;display:block;object-fit:contain;object-position:center;pointer-events:none}
...
.compare-preview .compare-clip::after{content:'';position:absolute;top:0;bottom:0;left:0;width:2px;background:var(--accent);box-shadow:0 0 14px rgba(74,222,128,.45)}
.compare-preview .compare-label{position:absolute;top:12px;padding:5px 9px;border:1px solid rgba(255,255,255,.12);background:rgba(9,9,11,.82);backdrop-filter:blur(8px);border-radius:5px;color:#fff;font:600 10px var(--mono);letter-spacing:.08em;text-transform:uppercase;z-index:12;pointer-events:none}
.compare-preview .compare-label.before{left:12px}.compare-preview .compare-label.after{right:12px}
.compare-preview .compare-handle{position:absolute;top:50%;left:50%;width:36px;height:36px;transform:translate(-50%,-50%);border:2px solid var(--accent);background:#09090b;border-radius:50%;display:flex;align-items:center;justify-content:center;z-index:13;box-shadow:0 0 0 4px rgba(9,9,11,.35),0 5px 18px rgba(0,0,0,.5);pointer-events:none}
...
.compare-preview .compare-bar{position:absolute;left:16px;right:16px;bottom:14px;height:3px;border-radius:3px;background:rgba(255,255,255,.18);z-index:14;pointer-events:none}
.compare-preview .compare-bar-fill{height:100%;width:50%;background:var(--accent);border-radius:3px}
.compare-preview .compare-hint{position:absolute;left:50%;bottom:24px;transform:translateX(-50%);font:500 9px var(--mono);color:rgba(255,255,255,.75);background:rgba(9,9,11,.78);padding:4px 7px;border-radius:4px;z-index:15;white-space:nowrap;pointer-events:none}
...
compare.innerHTML=`<img class="compare-after" alt="После обработки"><div class="compare-clip"><img class="compare-after" alt="После обработки"></div><span class="compare-label before">До</span><span class="compare-label after">После</span><div class="compare-handle" aria-hidden="true"></div><div class="compare-bar"><div class="compare-bar-fill"></div></div><div class="compare-hint">перетащите ползунок</div><input class="compare-range" type="range" min="0" max="100" value="50" aria-label="Сравнение до и после">`;
...
async function showCompare(){
 ...
 try{const blob=await buildAfter();if(token!==buildToken||!blob)return;const url=blobUrl(blob);after.src=url;clipped.src=url;compare.classList.add('show');requestAnimationFrame(()=>setPosition(range.value))}catch(e){console.warn('Safelight compare preview:',e)}
}
